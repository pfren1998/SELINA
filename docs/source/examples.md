...
Running the command showed above will output five useful files in the res/preprocess folder.
...
- `3. query_single_expr.txt` : expression matrix of query data for the prediction step, the first coolumn stores genes and the following columns store expression of all cells
...
This step will output eight files in the res/predict/example1 folder:
...
- `2. example1_probability.txt` : probablity of cells predicted as each of the reference cell types
...
- `5. example1_prob.txt` : prediction probability of each cell, each row represents one cell. Clusters with lower prediction probability may contain novel cell types that are not included in the reference
...
- `6. example1_cluster_prob.png` : box plot indicating the prediction probability distribution of cells in each cluster
  ![image](./_images/example1_cluster_prob.png)

- `7. example1_unknown_percent.txt` : the percentage of cells that are assigned unknown in each cluster, each row represents one cluster. Clusters with higher unknown percentage may contain novel cell types that are not included in the reference

```
unknown_percent	cluster
0.00814332247557003	0
0.0101663585951941	1
0	2
0.0126091173617847	3
0.017458777885548	4
0	5
0.126623376623377	6
0	7
0.0146627565982405	8
```

- `8. example1_unknown_percent.png` : bar plot indicating the percentage of cells that are assigned unknown in each cluster
  ![image](./_images/example1_unknown_percent.png)

## Example2

This example shows how to predict for data from tissues with disease.

### 1. data

The data used here is one dataset with non-small-cell lung carcinoma(NSCLC) from (Phenotype molding of stromal cells in the lung tumor microenvironment)[https://www.nature.com/articles/s41591-018-0096-5] in mtx format. Model here used to predict is one pre-trained model that was trained on immune cells of various tissues and lung-specific cells from SLEINA reference.
...
![image](./_images/example2_pred.png)

`2. example2_cluster_prob.png`
![image](./_images/example2_cluster_prob.png)

`3. example2_unknown_percent.png`
![image](./_images/example2_unknown_percent.png)

Here we can see in the disease data, there are some clusters with higher unknown percentage and lower
lower prediction probability, indicatingg that they are the malignant clusters or clusters with novel cell types
